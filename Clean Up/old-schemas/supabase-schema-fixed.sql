-- Fixed Database Schema for Graham Stephen Build Backend
-- This script sets up the database schema matching the actual LinkedIn API responses
...
-- Create commenters table (updated to match actual LinkedIn API response)
CREATE TABLE IF NOT EXISTS commenters (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID NOT NULL REFERENCES research_sessions(id) ON DELETE CASCADE,
    linkedin_id VARCHAR(255) NOT NULL,
    comment_id VARCHAR(255), -- LinkedIn comment ID
    name VARCHAR(255),
    headline TEXT,
    profile_url TEXT,
    ...
    comment_timestamp BIGINT, -- Store original timestamp
    is_edited BOOLEAN DEFAULT FALSE,
    is_pinned BOOLEAN DEFAULT FALSE,
    comment_url TEXT,
    relevance_score DECIMAL(5,2) DEFAULT 0,
    profile_data JSONB, -- Store full LinkedIn profile data
    stats_data JSONB, -- Store comment stats (reactions, replies)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- ('test@example.com', '$2b$12$dummy.hash.for.testing', 'Test', 'User');

-- Note: RLS policies removed as they conflict with JWT-based authentication
