...
-- Create scoring events table for analytics (optional)
...
-- Research sessions are owned by users
CREATE POLICY "Users can view own sessions" ON research_sessions FOR ALL USING (auth.uid()::text = user_id::text);

-- Commenters belong to sessions owned by users
CREATE POLICY "Users can view commenters from own sessions" ON commenters FOR ALL 
USING (EXISTS (SELECT 1 FROM research_sessions WHERE research_sessions.id = commenters.session_id AND research_sessions.user_id::text = auth.uid()::text));

-- User feedback belongs to the user who created it
CREATE POLICY "Users can view own feedback" ON user_feedback FOR ALL USING (auth.uid()::text = user_id::text);